...
## Current Authoritative Direction - 23 July 2026

The sole live experiment roadmap is
`docs/dissertation_loao_mainline_lock_2026_07_23.md`. It supersedes earlier
completion roadmaps, router/hybrid pivots, QLoRA launch queues, and experiment
priority notes.

The approved thesis mainline studies taxonomy generalisation under increasing
difficulty:

1. supplied-candidate single-unseen LOAO;
2. generalized prediction over seen and unseen aspects;
3. dual-unseen prediction with asymmetric label descriptions (`NN`, `DN`,
   `ND`, `DD`);
4. multi-aspect parent-group holdout; and
5. compound organisation plus taxonomy shift.

All active methods must use aspect-conditioned sentiment and pair-set output.
The main new intervention is a frozen human-authored minimal definition that
uses only canonical taxonomy names and hierarchy. The dual-unseen description
crossover is core evidence. Completed historical results below retain their
recorded scope but do not define the next task.

## 1. One-Sentence Summary

This project evaluates fine-grained aspect-sentiment systems under progressively
harder taxonomy change, from one supplied unseen aspect to incomplete
descriptions, multiple unseen aspects, and compound organisation shift.
...
Completed:
  ...
  final held-out-aspect Qwen LoRA runner with manifest logging
  tiny local held-out-aspect Qwen LoRA smoke test
  fixed held-out-aspect Qwen LoRA validation/test run
  single-fold all-row Qwen LoRA LOAO validation-gated pilot
  full 12-fold Qwen LoRA LOAO command/launch plan
...
Not yet completed:
  revised Qwen absence-calibration objective for all-row LOAO
  target full-LOAO GPU environment/storage confirmation after the revised objective is validated
  full Qwen LoRA all-row LOAO robustness evaluation
```

The current full Qwen LOAO robustness result is still the zero-shot prompt baseline, not a fine-tuned Qwen LOAO claim. The full all-row LOAO test mean is `0.1212` pair samples F1 and `0.3378` pair micro F1. Positive-gold rows are much stronger (`0.8194` pair samples F1 mean and `0.8659` pair micro F1 mean), so the main Qwen zero-shot weakness is empty-gold absence calibration. The final LoRA runner and tiny smoke test show that local QLoRA loading, adapter saving, indexed JSON generation, candidate-ID mapping, metrics, and manifest logging work on the RTX 5050 Laptop GPU. A full fixed held-out-aspect Qwen LoRA validation/test run is complete: test pair samples F1 `0.5528`, pair micro F1 `0.5552`, pair macro F1 `0.4393`, valid JSON `1.0000`, schema-valid `0.9964`. This is modest fixed-split adaptation evidence and remains below the strongest local fixed baseline. A later single-fold all-row LOAO pilot on `Company brand: Competitor` tested grouped SFT, conservative prompting, singleton neg1, neg0.25, and neg0.10 absence-aware variants. The best validation branch was singleton neg0.10 with pair micro F1 `0.1900`, precision `0.1667`, recall `0.2209`, and valid/schema rates `1.0000/1.0000`; it remained below the same-fold Qwen zero-shot (`0.2397`) and local DistilBERT (`0.2490`) validation baselines. Therefore the full 12-fold fine-tuned Qwen LOAO should not be launched with the current SFT recipe; it needs a revised absence-calibration objective first.
...
The active completion roadmap is now `docs/thesis_completion_roadmap.md`. Thesis-ready result tables, the cascade score/margin uncertainty check, Qwen LoRA runner readiness, the tiny Qwen LoRA smoke test, the fixed held-out-aspect Qwen LoRA validation/test run, the full LOAO launch plan, the single-fold all-row Qwen LoRA validation-gated pilot, and the local-to-Qwen score-distance routing diagnostic are complete. Full fine-tuned Qwen LoRA LOAO remains unrun because the current SFT recipe failed the single-fold validation gate; stronger GPU access alone is not enough to restart that path.

The current modelling direction has now pivoted from direct Qwen JSON-SFT replacement to local-to-Qwen hybrid routing. The source-of-truth note is `docs/qwen_local_hybrid_direction.md`. The practical rule is: DistilBERT should be treated as the cheap calibrated gate, while Qwen should be used selectively as a semantic judge for uncertain or unfamiliar unseen-aspect cases. The completed asymmetric global score-distance router reaches `0.3900` test mean pair micro F1 with Qwen called on `29.1%` of rows, improving over the previous `score_abs_replace_le_0.05` gate at `0.3800`. The expanded-grid per-aspect validation-selected diagnostic reaches `0.4178` with Qwen called on `34.4%` of rows, but remains an upper-bound diagnostic only. Do not launch full 12-fold Qwen LoRA LOAO with the current grouped/singleton JSON-SFT recipe merely because a larger GPU is available.
...
2. What GPU environment should be used for full Qwen experiments?
   - Local QLoRA works, fixed held-out-aspect Qwen LoRA is complete, local zero-shot Qwen LOAO is complete, and a single-fold all-row Qwen LoRA pilot is complete.
   - The current SFT recipe did not beat same-fold zero-shot/local validation baselines, so stronger GPU access alone is not enough; full 12-fold fine-tuned LOAO should wait for a revised absence-calibration objective.
...
5. Which LLM stage should follow the completed non-LLM held-out-aspect baseline?
   ...
   - This was the pre-23-July position. The approved next work is now the
     taxonomy-generalisation difficulty ladder in
     `docs/dissertation_loao_mainline_lock_2026_07_23.md`.
   - Candidate-aspect descriptions, the Gemini-assisted qualitative error taxonomy, thesis-ready tables, cascade score/margin uncertainty, Qwen LoRA runner readiness, tiny Qwen LoRA smoke testing, the fixed held-out-aspect Qwen LoRA validation/test run, the Qwen LoRA full LOAO launch plan, a single-fold all-row Qwen LoRA validation-gated pilot, local-to-Qwen score-distance routing, asymmetric global routing, and F1/call-rate Pareto reporting are now complete. The global-router contribution is sufficient for the thesis-facing local-to-Qwen method unless a supervisor requests more evidence. Per-aspect routing should remain an upper-bound diagnostic, and candidate-wise Qwen judgement plus descriptions are deferred unless a new Qwen-inference question is explicitly needed.

## 16. Immediate Next Steps

Use only the checklist in
`docs/dissertation_loao_mainline_lock_2026_07_23.md`. The next unfinished block
is description-resource audit and freeze.

The approved order is:

1. freeze the leakage-safe minimal description resource;
2. build and test the common candidate-pair stress-test infrastructure;
3. complete strict zero-label Level 1;
4. run generalized single-unseen Level 2;
5. run the core dual-unseen `NN/DN/ND/DD` crossover;
6. run gated parent-group and compound-shift stress tests; and
7. add final QLoRA seeds, statistics, figures, and thesis text.

Completed Gemini, router, fixed-split, old Qwen JSON-SFT, and historical
description experiments remain evidence only. They are not alternative live
queues.
