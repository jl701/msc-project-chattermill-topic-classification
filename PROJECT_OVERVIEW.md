...
5. Which LLM stage should follow the completed non-LLM held-out-aspect baseline?
   ...
   - Candidate-aspect descriptions, the Gemini-assisted qualitative error taxonomy, thesis-ready tables, cascade score/margin uncertainty, Qwen LoRA runner readiness, tiny Qwen LoRA smoke testing, the fixed held-out-aspect Qwen LoRA validation/test run, the Qwen LoRA full LOAO launch plan, a single-fold all-row Qwen LoRA validation-gated pilot, and local-to-Qwen score-distance routing are now complete. Recommended order after this point: first run asymmetric score-distance routing and the F1/call-rate Pareto curve. These two items are considered sufficient for the thesis-facing local-to-Qwen contribution if completed cleanly. Per-aspect routing should remain an upper-bound diagnostic, and candidate-wise Qwen judgement plus descriptions are deferred unless the global-router work leaves a clear gap.
...
15. Treat the local-to-Qwen score-distance hybrid as the immediate Qwen modelling result. The strongest local DistilBERT LOAO predictions have been rerun with score, threshold-distance, and sentiment-confidence features, and existing Qwen zero-shot LOAO predictions have been reused for validation-selected routing. The next prioritised work is item 1, asymmetric score-distance routing, and item 2, cost-quality / F1-call-rate Pareto reporting. Use a unified global router as the headline method. Items 3-6 are not the next default: lightweight defer routing is lower priority, per-aspect routing is diagnostic only, and candidate-wise Qwen semantic judging with descriptions is deferred.
