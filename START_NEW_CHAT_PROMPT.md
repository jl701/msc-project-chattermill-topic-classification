...
- The next user-confirmed Qwen hybrid priorities are: 1. asymmetric score-distance routing; 2. F1/call-rate Pareto curve. These two are considered sufficient for the thesis-facing contribution if completed cleanly. Use one unified global router as the headline method. Per-aspect routing is diagnostic only, and candidate-wise Qwen semantic judging plus aspect descriptions are deferred unless items 1/2 leave a clear gap.
...
11. Use `docs/thesis_completion_roadmap.md` and `docs/qwen_local_hybrid_direction.md` as the current task order. Thesis-ready tables/figure data, cascade score/margin uncertainty, Qwen LoRA runner readiness, tiny smoke testing, the fixed held-out-aspect Qwen LoRA validation/test run, the full Qwen LoRA LOAO launch plan, the single-fold all-row Qwen LoRA validation-gated pilot, and local-to-Qwen score-distance routing are complete. Remaining Qwen work should first run item 1, asymmetric score-distance routing, and item 2, F1/call-rate Pareto reporting. Treat these two as enough for the main thesis method if cleanly completed. Use a unified global router as the headline method; keep per-aspect routing diagnostic only; defer candidate-wise Qwen judging and descriptions.
