...
This note records the current modelling pivot after the fixed held-out-aspect Qwen LoRA run, the single-fold all-row Qwen LoRA validation pilot, and the first offline local-to-Qwen LOAO cascade diagnostic.
...
This is motivated by the stronger local-to-Gemini fixed-split cascade and the first local-to-Qwen LOAO diagnostic, where a simple validation-selected local/Qwen agreement gate improved mean test pair micro F1 over both local-only and Qwen-only:
...
This is the first method to run because it reuses existing Qwen LOAO predictions and only requires rerunning/exporting the local DistilBERT LOAO scores.
...
This is useful because LOAO diagnostics show that aspect detection and sentiment assignment are separable bottlenecks.

### 3. Per-Aspect Validation Routing

Some held-out aspects are better handled by Qwen-led policies, while others need local/Qwen agreement. A validation-selected per-aspect policy is currently optimistic, but it gives a useful upper bound and can be turned into a deployable method if the routing rule is frozen from validation features only.
...
This better matches Qwen's semantic strength and directly targets absence calibration. It should be considered after the no-new-Qwen-call score/margin routing analysis, because it will require new Qwen generation or a revised training objective.
...
## First Registered Experiment In This Direction
