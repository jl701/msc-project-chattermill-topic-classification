...
Level 1-4 taxonomy-generalisation ladder, deferred Level 5, and the only live
TODO list.
...
The generalised single-unseen, dual-unseen `NN/DN/ND/DD` crossover, secondary
Level 3 `RR - DD` rich-guidance comparison, and parent-group protocols are
approved plans but have not yet produced formal results. Their configs,
resources, hashes, and execution rules are pre-registered; compound shift is
deferred.
...
| Active LOAO table | Authoritative aspect-conditioned baseline registry and table | Complete and frozen | `python .\scripts\build_active_loao_baseline_table.py` | `configs/experiments/loao_active_baseline_registry_v1.json`, `docs/thesis_result_tables.md`, `docs/thesis_figure_data/loao_active_baselines_v1.csv` | Contains exactly seven active target-calibrated all-row LOAO rows. Validation rejects global/shallow sentiment, legacy method IDs, retired headline values, changed method order, and disagreement with the frozen four-method similarity export. |
| Strict TF-IDF/Qwen router | Aspect-conditioned validation-selected replacement router | Complete negative result; dropped from mainline | Run validation then test with the commands in `docs/experiments/loao_strict_tfidf_aspect_qwen_router_v1.md` | `configs/experiments/loao_strict_tfidf_aspect_qwen_router_v1.json`, `scripts/analyse_strict_tfidf_aspect_qwen_router.py`, focused tests, experiment report, tracked summary/per-aspect CSVs, ignored detailed output under `outputs/analysis/loao_strict_tfidf_aspect_qwen_router_v1/` | Registered before execution; no new inference. All twelve folds aligned exactly. Validation selected the zero-call local-only policy. Test F1 therefore remained `0.385640`, gain `0`, call rate `0`; failed the registered `+0.01` gate. No quality–call-rate figure. |
| Taxonomy architecture | Matched one-stage independent pairs versus true two-stage aspect-then-sentiment | Complete validation-only sequential comparison | `python .\scripts\run_taxonomy_matched_one_vs_two_stage_validation.py --method e5_base_v2 --local-files-only`; `python .\scripts\run_taxonomy_matched_one_vs_two_stage_validation.py --method strict_train_only_tfidf`; `python .\scripts\audit_taxonomy_matched_one_vs_two_stage_validation.py --write .\outputs\experimental\taxonomy_matched_one_vs_two_stage_validation_v1\audit.json` | `configs/experiments/taxonomy_matched_one_vs_two_stage_validation_v1.json`, `docs/experiments/taxonomy_matched_one_vs_two_stage_validation_v1_results.md`, `docs/thesis_figure_data/taxonomy_matched_one_vs_two_stage_validation_v1.csv`, ignored fold artifacts under `outputs/experimental/taxonomy_matched_one_vs_two_stage_validation_v1/` | Same 12 L2 folds, 1,057 validation rows, 36 name-plus-description candidates, and seen-only selection. Capped-two is the primary two-stage decoder. TF-IDF F1 `0.2196 -> 0.2773` (`+0.0577`, 11/0/1); E5 `0.1860 -> 0.2222` (`+0.0362`, 9/0/3). 24/24 folds, 0 failure/non-finite/test-contract. Official test remained sealed. |
