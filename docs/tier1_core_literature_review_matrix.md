...
| Open-domain topic classification | Ding et al., 2022, *Towards Open-Domain Topic Classification*. Local: `Project_Preparation/Recent_Paper/2306.17290_Towards_Open_Domain_Topic_Classification.pdf` is the 2023 arXiv copy. External: NAACL 2022 System Demonstrations / arXiv `2306.17290`. | Introduces user-defined taxonomies at inference time and label-aware zero-shot classification. It explicitly treats labels as semantic inputs. | Topic classification is not ABSA, does not model sentiment, and is not FABSA-specific. | This is the main conceptual bridge from fixed-label FABSA classifiers to candidate-label taxonomy-shift ABSA. |
...
## Foundational Citation Update 2026-06-30

This update fills the first set of foundation references that should be useful regardless of Aji's final direction feedback. These sources should be used to strengthen the literature review's methodological spine, not to open deep Tier 2 branches.

| Foundation Area | Added / Prioritised Sources | Why Add Now | How To Use |
| --- | --- | --- | --- |
| Multi-label evaluation metrics | Tsoumakas and Katakis, 2007/2008; Zhang and Zhou, 2014; Sokolova and Lapalme, 2009, *A Systematic Analysis of Performance Measures for Classification Tasks*, DOI `10.1016/j.ipm.2009.03.002`. | The dissertation must justify sample-level F1, micro F1, macro F1, and threshold diagnostics before any final result table is written. | Use Zhang and Zhou / Tsoumakas and Katakis for multi-label framing; use Sokolova and Lapalme for general metric definitions and cautions. |
| Cross-domain ABSA / domain shift | Blitzer et al., 2007; Ramponi and Plank, 2020; Zhang et al., 2023 ABSA survey; Yu et al., 2023, *Cross-Domain Data Augmentation with Domain-Adaptive Language Modeling for Aspect-Based Sentiment Analysis*, ACL Anthology `2023.acl-long.81`; Li et al., 2024, *Structure-aware Generation Model for Cross-Domain Aspect-based Sentiment Classification*, ACL Anthology `2024.lrec-main.1335`. | The current held-out organisation protocol needs direct support from domain-shift and cross-domain ABSA literature, not only generic sentiment adaptation. | Use this to explain why held-out organisation is a separate evaluation axis from held-out aspect. Do not deep-dive into adaptation algorithms unless the method later changes. |
| Dynamic taxonomy / changing label sets | Godbole et al., 2005, *Text Classification with Evolving Label-Sets*, IEEE ICDM DOI `10.1109/ICDM.2005.143`; Wohlwend et al., 2019, *Metric Learning for Dynamic Text Classification*, ACL Anthology `D19-6116` / DOI `10.18653/v1/D19-6116`. | This fills the direct evidence gap that text-classification label sets can evolve after initial training and that fixed-output classifiers are not ideal when labels are added or removed. | Use these in the taxonomy-shift subsection before candidate-label / zero-shot literature. They justify the problem setting; Ding/Yin/Pappas/Chai then justify label semantics and candidate-label modelling. |
| Candidate-label / label semantics / zero-shot classification | Chang et al., 2008, *Importance of Semantic Representation: Dataless Classification*, AAAI; Yin et al., 2019; Ding et al., 2022; Pappas and Henderson, 2019, *GILE: A Generalized Input-Label Embedding for Text Classification*, TACL / ACL Anthology `Q19-1009`; Chai et al., 2020, *Description Based Text Classification with Reinforcement Learning*, ICML / PMLR `v119/chai20a`. | This is the core theoretical support for representing labels as input text instead of fixed classifier-head indices. | Use these to justify candidate aspects, indexed candidate labels, and possible label descriptions. Keep the focus on label semantics, not prompt engineering details. |
| Structured-output LLM reliability | Willard and Louf, 2023; Tam et al., 2024; Geng et al., 2025, *JSONSchemaBench: A Rigorous Benchmark of Structured Outputs for Language Models*, arXiv `2501.10868`; Beurer-Kellner et al., 2023, *Prompting Is Programming: A Query Language for Large Language Models*, arXiv `2212.06094`. | Qwen/Gemini outputs must be evaluated for JSON/schema validity, not only F1. This is true even before deciding whether Gemini becomes central. | Use these references to motivate structured output diagnostics: parse validity, schema violations, hallucinated labels, invalid sentiments, and the possible trade-off between constraints and task performance. |

The second and third classes of references remain deferred:

- Qwen/Gemini-specific fine-tuning, token accounting, and hosted-model cost literature should wait until Aji responds and the next modelling branch is selected.
- Joint aspect-sentiment pair-scoring and deployment-governance literature should stay as placeholders unless those branches become central.
...
These are remaining or partially filled Tier 1 gaps. They can be filled selectively rather than by deep rabbit-hole reading:
...
| Dynamic taxonomies in industrial classification | Basic direct support has been added via evolving-label-set and dynamic-text-classification literature. More industrial taxonomy evidence is optional if the final discussion needs it. | Added: Godbole et al. 2005; Wohlwend et al. 2019. Optional search: `taxonomy shift text classification`, `dynamic taxonomy text classification`, `label taxonomy evolution NLP`. |
